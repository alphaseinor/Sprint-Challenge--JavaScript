...
Dinosaur.prototype.roar = function(roarSound) {
  return roarSound;
};
...
let universities = graduates.map(x => x.university);
...
let contactInfo = graduates.map(x => x.first_name + ' ' + x.email);
...
let uni = graduates.filter(x => x.university.includes('Uni'));
console.log(uni);
...
zooAnimals.forEach(element => {
  animalNames.push(
    `Name: ${element.name}, Scientific${element.scientific_name}.`
  );
});
...
*/
let lowerPopulation = zooAnimals.filter(x => x.population < 5);
